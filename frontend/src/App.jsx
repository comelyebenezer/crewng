...
function JoinModal({ onClose, onDone }) {
  ...
  const validate=()=>{
    ...
    if(step===2){if(!f.name.trim())e.name="Full name required";if(!f.email.includes("@"))e.email="Valid email required";if(f.nin.replace(/\D/g,"").length<11)e.nin="NIN must be 11 digits";if(!f.phone)e.phone="Phone required";if(!f.location)e.location="Location required";}
    ...
    <div className="overlay" onClick={onClose}>
      <div className="mbox" onClick={e=>e.stopPropagation()} style={{maxWidth:500,maxHeight:"94vh",overflowY:"auto"}}>
        ...
        <div style={{padding:"18px 20px 24px",display:"flex",flexDirection:"column",gap:14}}>
          ...
          {step===2&&<><p style={{fontSize:13.5,fontWeight:700}}>Personal Information</p><Field k="name" label="Full Name (must match NIN exactly)" placeholder="e.g. Ngozi Chioma Okafor" required/><Field k="email" label="Email Address" placeholder="your@email.com" type="email" required/><Field k="nin" label="NIN — National ID Number" placeholder="11-digit NIN" required/><div style={{background:"rgba(245,200,66,.07)",border:"1px solid rgba(245,200,66,.18)",borderRadius:9,padding:"9px 12px",fontSize:10.5,color:"rgba(255,255,255,.5)",lineHeight:1.65}}>🔒 Your NIN is one-way encrypted and never displayed publicly.</div><Field k="phone" label="Phone Number" placeholder="+234…" required/><Field k="location" label="State / City" placeholder="e.g. Lagos" required/><Btns/></>}
  ...
  return (
    <div className="pcard" onClick={()=>onClick(p)}>
      <div style={{position:"relative",width:"100%",paddingBottom:"95%",background:p.bg,flexShrink:0}}>
        ...
        {p.verified&&<div style={{position:"absolute",top:9,right:9,background:"rgba(14,14,14,.85)",color:cat.color,fontSize:8.5,fontWeight:700,padding:"3px 8px",borderRadius:50,border:`1px solid ${cat.color}44`}}>✓ NIN</div>}
    ...
    <div className="overlay" onClick={onClose}>
      <div className="mbox" onClick={e=>e.stopPropagation()} style={{maxWidth:490,maxHeight:"92vh",overflowY:"auto"}}>
        <div style={{position:"relative",background:p.bg,paddingBottom:"48%",borderRadius:"22px 22px 0 0",overflow:"hidden"}}>
          ...
          {p.verified&&<div style={{position:"absolute",top:13,left:13,background:"rgba(14,14,14,.85)",color:cat.color,fontSize:10,fontWeight:700,padding:"4px 11px",borderRadius:50,border:`1px solid ${cat.color}44`}}>✓ NIN + Face Verified</div>}
    ...
    <div style={{minHeight:"100vh",background:"#0a0a0a",color:"#fff",fontFamily:"'Sora',sans-serif"}}>
      ...
      <div style={{padding:"24px 5%",maxWidth:1100,margin:"0 auto"}}>
        ...
        {tab==="overview"&&<div style={{display:"flex",flexDirection:"column",gap:18}}>
          ...
          {[{t:"New booking",d:"Chidi Okafor booked Ngozi as Waitress · ₦45,000",time:"4 min ago",c:"#34d399"},{t:"Rating submitted",d:"Booking CNG-G7H8I9 rated · Payment released ₦250,000",time:"22 min ago",c:"#f5c842"},{t:"NIN Verified",d:"Ngozi Okafor — face matched successfully",time:"1 hr ago",c:"#60a5fa"},{t:"Account flagged",d:"Suspicious login on user #4821",time:"3 hrs ago",c:"#ef4444"}].map((a,i)=>(
